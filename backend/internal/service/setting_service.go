...
// GlobalTempUnschedulableConfig is the global account error handling rule config.
type GlobalTempUnschedulableConfig struct {
	Enabled bool                    `json:"enabled"`
	Rules   []TempUnschedulableRule `json:"rules"`
}

// GetGlobalTempUnschedulableRules reads global temp-unschedulable rules.
func (s *SettingService) GetGlobalTempUnschedulableRules(ctx context.Context) GlobalTempUnschedulableConfig {
	...
	if raw := strings.TrimSpace(vals[SettingKeyAccountTempUnschedulableRules]); raw != "" {
...
// SetGlobalTempUnschedulableRules writes global temp-unschedulable rules.
