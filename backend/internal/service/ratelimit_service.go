...
// matchTempUnschedulableRules matches temp-unschedulable rules.
// Keywords are optional: empty keywords match by status code alone (local behavior);
// configured keywords must appear in the response body.
func matchTempUnschedulableRules(statusCode int, responseBody []byte, rules []TempUnschedulableRule) []tempUnschedulableRuleMatch {
	if statusCode <= 0 || len(rules) == 0 {
		return nil
	}
	body := responseBody
...
func (s *RateLimitService) tryTempUnschedulable(ctx context.Context, account *Account, statusCode int, responseBody []byte, requestedModel ...string) bool {
	...
	for _, match := range matchTempUnschedulableRules(statusCode, responseBody, rules) {
